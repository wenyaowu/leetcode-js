...
On any two opposite side of the cube, the number of pips adds up to 7; that is, 
there are three pairs of opposite sides: 1 and 6, 2 and 5, and 3 and 4.
...
Input: A = [1, 1, 6]
Output: 2
Explanation: The only optimal answer is to rotate the last die so that it shows one pip. 
It is necessary to use two rotations to achieve this.
Example 3:
...
function rollDice(A) {
  const rollsToNumber = new Array(6).fill(0);
  for (let dice of A) {
    for (let i = 0; i < 6; i++) {
      let current = i + 1;
      rollsToNumber[i] += rolls(dice, current);
    }
  }
  return Math.min(...rollsToNumber);
}

function rolls(num1, num2) {
  if (num1 === num2) {
    return 0;
  }
  return 7 - num1 === num2 ? 2 : 1;
}
